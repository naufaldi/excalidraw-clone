...
import { createDefaultBoard, createDefaultPreferences } from "./shared/index.js";

function toStoredElement(element: Element): Omit<Element, "createdAt" | "updatedAt"> {
...
export async function createBoard(data: Partial<Board>): Promise<Board> {
  ...
  const boardData = {
    ...createDefaultBoard(),
    ...data,
    id: data.id || `board-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  };
...
export async function duplicateBoard(boardId: string, newName?: string): Promise<Board> {
  ...
  const originalDoc = await collection.findOne({ selector: { id: boardId } }).exec();
 ...
 */
export async function addElementToBoard(boardId: string, element: Element): Promise<Board> {
  const collection = getBoardsCollection();
...
/**
 * Add multiple elements to a board at once
 * More efficient than adding each element separately
 * Useful for paste operations
 */
export async function addMultipleElementsToBoard(
  boardId: string,
  elements: Element[],
): Promise<Board> {
  const collection = getBoardsCollection();
  ...
  // Convert all elements to stored format (remove Date objects)
  const storedElements = elements.map(toStoredElement);

  const updatedDoc = await doc.incrementalPatch({
    elements: [...doc.elements, ...storedElements],
  });

  console.log("[RxDB] Multiple elements added to board:", boardId, elements.length, "elements");
  return updatedDoc.toJSON() as Board;
}
 ...
 */
export async function deleteElementFromBoard(boardId: string, elementId: string): Promise<Board> {
  const collection = getBoardsCollection();
...
export async function deleteMultipleElementsFromBoard(
  ...
  const elements = doc.elements.filter((el: any) => !elementIds.includes(el.id));
 ...
 */
export async function clearAllElementsFromBoard(boardId: string): Promise<Board> {
  const collection = getBoardsCollection();
...
export async function updateBoardPreferences(
  ...
  const updatedDoc = await doc.incrementalPatch({
    preferences: {
      ...
      gridEnabled: preferences?.gridEnabled ?? doc.preferences?.gridEnabled ?? true,
    },
  });
...
export async function createBoardWithElements(
  ...
  const collection = getBoardsCollection();
  const doc = await collection.insert(boardData);
  console.log("[RxDB] Board created with", elements.length, "elements:", doc.id);
  return doc.toJSON() as Board;
}
 ...
 */
export async function createMultipleBoards(boards: Array<Partial<Board>>): Promise<Board[]> {
  const collection = getBoardsCollection();

  const boardData = boards.map((board) => ({
    ...createDefaultBoard(),
    ...board,
    id: board.id || `board-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  }));
...
export async function deleteMultipleBoards(boardIds: string[]): Promise<void> {
  ...
  const docs = await collection.find({ selector: { id: { $in: boardIds } } }).exec();
