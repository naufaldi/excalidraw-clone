 ...
 */
export const elementSchema: RxJsonSchema<
...
export const boardSchema: RxJsonSchema<Omit<Board, "createdAt" | "updatedAt">> =
	{
		...
		/**
		 * Schema version (RxDB collection schema version).
		 * Increment this when changing the JSON schema to enable migrations.
		 * Version 1: Initial schema
		 * Version 2+: Future versions
		 */
		version: 1,
		primaryKey: "id",
		type: "object",
...
/**
 * Migration strategies for the `boards` collection.
 *
 * NOTE: These migrations are intentionally no-ops for now because the v1 schema
 * change that triggered DB6 was a constraint-only change (e.g. adding maxLength)
 * and existing documents remain valid.
 */
export const boardMigrationStrategies: MigrationStrategies<
...
> = {
	...
	// Note: "id" is primaryKey so it's automatically indexed, no need to declare
};
 ...
 */
export function createDefaultPreferences(): Omit<
	UserPreferences,
	"createdAt" | "updatedAt"
> {
	return {
