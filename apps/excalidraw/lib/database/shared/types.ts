 ...
 * Note: createdAt/updatedAt managed by RxDB via _meta.lwt
 */
export interface Element {
	...
	/**
	 * Optional timestamps used by the UI while drawing.
	 * When persisted into RxDB, these are not stored; RxDB internal `_meta.lwt`
	 * is used for last-write-time instead.
	 */
	createdAt?: Date;
	updatedAt?: Date;
}
 ...
 */
export function isPenElement(
	element: Element,
): element is Element & { points: Point[] } {
...
export function isShapeElement(
	element: Element,
): element is Element & { width: number; height: number } {
	return (
...
export function isTextElement(
	element: Element,
): element is Element & { text: string; fontSize: number } {
	return (
		element.type === "text" &&
